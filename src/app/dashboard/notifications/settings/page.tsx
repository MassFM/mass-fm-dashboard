  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-blue-50 rounded-2xl p-5 border border-blue-100">
        <h3 className="text-sm font-bold text-blue-700 mb-2">ℹ️ Setup Database</h3>
        <p className="text-xs text-blue-600 mb-2">Buat tabel berikut di Supabase SQL Editor:</p>
        <pre className="text-[10px] bg-white rounded-xl p-3 overflow-x-auto text-blue-800 font-mono">
{`CREATE TABLE notification_settings (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  auto_notify_enabled boolean DEFAULT false,
  notify_before_minutes int DEFAULT 10,
  notify_topic text DEFAULT 'jadwal_update',
  last_auto_sent_at timestamptz,
  created_at timestamptz DEFAULT now()
);

-- Insert default row
INSERT INTO notification_settings (auto_notify_enabled) VALUES (false);`}
        </pre>
        <p className="text-xs text-blue-600 mt-3 mb-1">Untuk pengiriman otomatis, gunakan Supabase pg_cron atau external cron:</p>
        <pre className="text-[10px] bg-white rounded-xl p-3 overflow-x-auto text-blue-800 font-mono">
{`-- Contoh pg_cron (jalankan setiap 5 menit)
SELECT cron.schedule('auto_notify', '*/5 * * * *',
  $$SELECT net.http_post(
    url := 'https://mnjthnylygqxxgymakse.supabase.co/functions/v1/schedule-notification',
